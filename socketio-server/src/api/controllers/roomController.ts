...
export class RoomController {
  ...
  public async joinGame(@SocketIO() io: Server, @ConnectedSocket() socket: Socket, @MessageBody() message: any)
  {
    ...
    if (socketRooms.length > 0 ||(connectedSockets && connectedSockets.size === 2)) {
      socket.emit("room_join_error", {error: "Room is full please choose another room to play!",});
    }
    else {
  ...
  public async leaveGame(@SocketIO() io: Server, @ConnectedSocket() socket: Socket, @MessageBody() message: any)
  {
    ...
    if (socketRooms.length == 0 ||(connectedSockets && connectedSockets.size === 0)) {
      socket.emit("room_leave_error", {error: "Room is already empty!",});
    }
    else {
